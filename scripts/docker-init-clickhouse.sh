#!/usr/bin/env bash
# Apply ClickHouse schema one statement at a time (HTTP API rejects multi-statement bodies).
set -euo pipefail

CH="${CLICKHOUSE_URL:-http://clickhouse:8123}"

run_query() {
  curl -sf "${CH}/" --data-binary "$1" >/dev/null
  echo "OK: ${1:0:60}..."
}

echo "Applying ClickHouse schema to ${CH}..."

until curl -sf "${CH}/ping" >/dev/null; do
...
run_query "CREATE TABLE IF NOT EXISTS blamr.run_summaries (
    ...
    accuracy_score Float64, total_tokens Int64, total_cost_usd Float64, duration_ms Int64,
    agent_count Int32, started_at Int64, ended_at Int64,
    updated_at DateTime64(3) DEFAULT now64(3)
) ENGINE = ReplacingMergeTree(updated_at) ORDER BY (workspace_id, workflow_id, run_id)"

run_query "CREATE MATERIALIZED VIEW IF NOT EXISTS blamr.run_summaries_mv
TO blamr.run_summaries AS SELECT
    run_id, workflow_id, workspace_id, 'running' AS status, 0.0 AS accuracy_score,
    sum(tokens_in + tokens_out) AS total_tokens, sum(cost_usd) AS total_cost_usd,
    sum(latency_ms) AS duration_ms,
    uniqExact(from_agent) + uniqExact(to_agent) AS agent_count,
    min(timestamp_ms) AS started_at, max(timestamp_ms) AS ended_at, now64(3) AS updated_at
FROM blamr.causal_edges GROUP BY run_id, workflow_id, workspace_id"
