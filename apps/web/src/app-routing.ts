...
export function parseAppRoute(): AppRoute {
  ...
  if ((head === 'executions' || head === 'runs') && parts[1]) {
    if (parts[1] === 'agent' && parts[2] && parts[3]) {
      return {
        view: 'list',
        scopeWorkflowId: decodeURIComponent(parts[2]),
        scopeAgentId: decodeURIComponent(parts[3]),
      };
    }
    if (parts[1] !== 'failed' && parts[1] !== 'success') {
...
export function hashForRoute(
  ...
  switch (route.view) {
    ...
    case 'list': {
      if (route.scopeWorkflowId && route.scopeAgentId) {
        return `#/executions/agent/${encodeURIComponent(route.scopeWorkflowId)}/${encodeURIComponent(route.scopeAgentId)}`;
      }
